...
const createMockWordPack = (): WordPack => {
  ...
  return new WordPack({
    ...
    midnight: 'midnight',
    noon: 'noon',
    daysOfMonth: Array.from({ length: 31 }, (_, i) => `${i + 1}${getOrdinalSuffix(i + 1)}`),
  });
};
...
describe('ClockFormatter', () => {
  ...
  describe('getClockText', () => {
    it('should format midnight correctly', () => {
      ...
      const result = formatter.getClockText(date, false, false, TimeFormat.FORMAT_ONE, 5);
    ...
    it('should include date when showDate is true', () => {
      ...
      const result = formatter.getClockText(date, true, false, TimeFormat.FORMAT_ONE, 5);
    ...
    it('should include weekday when showWeekday is true', () => {
      const date = new Date('2024-01-01T15:00:00'); // Monday, Jan 1st, 3:00 PM
      const result = formatter.getClockText(date, true, true, TimeFormat.FORMAT_ONE, 5);
      expect(result).toContain('Monday');
    });

    it('should handle format two correctly', () => {
      const date = new Date('2024-01-01T15:15:00'); // 3:15 PM
      const result = formatter.getClockText(date, false, false, TimeFormat.FORMAT_TWO, 5);
