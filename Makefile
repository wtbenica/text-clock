...
## pr-create-draft     Create a draft PR for testing validation/workflow timing
pr-create-draft:
	...
	echo ""; \
	echo "To promote to ready for review: make pr-promote"; \
	echo "To release using this PR: make pr-use-existing"

## pr-create           Create a PR from current branch to main (use DRAFT=1 to create draft)
pr-create:
	# Create release PR from current branch (DRAFT=1 to create draft)
	@current_branch=$$(git rev-parse --abbrev-ref HEAD); \
	if [ "$$current_branch" = "main" ]; then \
		echo "ERROR: You're on main branch. This command should be run from a development branch."; \
		exit 1; \
	fi; \
	@echo "Creating release PR from branch: $$current_branch"; \
	@DRAFT=$${DRAFT:-0}; \
	if [ "$$DRAFT" = "1" ]; then \
		$(MAKE) pr-create-draft; \
	else \
		pr_output=$$(gh pr create --base main --head "$$current_branch" --title "Release v$(CURRENT_VERSION)" --body "Automated release PR for version $(CURRENT_VERSION)" --fill 2>&1) || true; \
		echo "PR creation output:"; echo "$$pr_output"; \
	fi

## pr-promote          Promote current branch's draft PR to ready for review
pr-promote:
	...
	echo "Create a draft PR first with: make pr-create-draft"; \
		exit 1; \
	fi; \
...
## pr-use-existing     Complete release using an existing PR (promote if draft, wait & merge, then release)
pr-use-existing:
	...
	if [ -z "$$pr_number" ] || [ "$$pr_number" = "null" ]; then \
	echo "ERROR: No open PR found for branch $$current_branch"; \
	echo "Create a PR first with: make pr-create-draft"; \
		exit 1; \
	fi; \
	pr_status=$$(gh pr view "$$pr_number" --json isDraft --jq '.isDraft'); \
